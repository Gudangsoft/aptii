...
class CertificateParticipant extends Component
{
    ...
    public function render()
    {
        ...
        $data = CertificateModel::where('user_id', auth()->user()->id)->orderByDesc('created_at')->paginate($this->limitPerPage);

        $this->emit('postStore');
        $this->dispatchBrowserEvent('iconLoad');

        $this->bulkDisabled = count($this->selectData) < 1;
        return view('livewire.prosiding.certificate-participant', [
            'data' => $data,
        ]);
    }
