    ...
    <div class="content-header row">
        <div class="content-header-left col-12 mb-2">
            <div class="row breadcrumbs-top">
                <div class="col-7">
                    <h2 class="content-header-title float-left mb-0">Panduan User</h2>
                    <div class="breadcrumb-wrapper">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">Panduan User
                            </li>
                        </ol>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-lg-6 col-sm-12 mt-1">
                                <button class="btn-icon btn btn-primary btn-round" data-toggle="modal" data-target="#create-modal-bidangilmu"><i data-feather="plus-circle"></i> Tambah</button>
                                    ...
                                    <div class="modal-dialog modal-dialog-centered" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h4 class="modal-title" id="myModalLabel16">Tambah Panduan</h4>
                                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                            </div>
                                            <form wire:submit.prevent='createGuide' enctype="multipart/form-data">
                                                <div class="modal-body">
                                                    <div class="form-group">
                                                        <label><h5>File PDF</h5></label>
                                                        <input type="file" wire:model.defer='file_guide' class="form-control" placeholder="File" required>
                                                    </div>
                                                    <div class="form-group">
                                                        <label><h5>Hak Akses</h5></label>
                                                        <div class="demo-inline-spacing">
                                                            @foreach ($roles as $item)
                                                                <div class="custom-control custom-control-primary custom-checkbox">
                                                                    <input type="checkbox" class="custom-control-input" id="colorCheck{{ $loop->index }}" wire:model.defer="category" value="{{ $item->id }}" />
                                                                    <label class="custom-control-label" for="colorCheck{{ $loop->index }}">{{ $item->name }}</label>
                                                                </div>
                                                                {{-- <input  type="checkbox" class="form-control" wire:model.defer="category" value="{{ $item->name }}"> --}}
                            ...
                            <tbody>
                                @forelse ($data as $row)
                                    <tr>
                                        ...
                                        <td>
                                            <span class="font-weight-bold">{{ $row->file }}</span>
                                            @if ($row->status < 1)
                                                <i class="text-danger" data-feather="eye-off"></i>
                                            @endif
                                        </td>
                                        <td>
                                            @foreach ($row->access($row->category) as $name)
                                                <span class="badge badge-primary">{{ strtoupper($name) }}</span>
                                            @endforeach
                                        </td>
                                        <td>
                                            @if ($row->status == true)
